"use client";
// Marks this as a client-side component in Next.js (runs in the browser).
...
// Represents the shape of a user's profile in the database
...
// Represents the user's dating preferences
export interface UserPreferences {
  age_range: { min: number; max: number }; // Minimum and maximum age preference
  distance: number; // Max distance (in km) for matches
  gender_preference: ("male" | "female" | "other")[]; // Gender preferences
}
...
export default function ProfilePage() {
  // State to hold profile data, loading state, and error messages
  ...
  useEffect(() => {
    async function loadProfile() {
      try {
        const profileData = await getCurrentUserProfile(); // Call server action
        console.log(profileData);

        if (profileData) {
          setProfile(profileData); // Store profile in state
        } else {
          setError("Failed to load profile"); // Handle no profile case
        }
      } catch (err) {
  ...
  // ----------------------
  return (
    <div className="bg-gradient-to-br from-pink-50 to-red-50 min-h-screen">
